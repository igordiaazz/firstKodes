...
export const GlareCard = ({
  ...
  const backgroundStyle = {
    ...
    "--rainbow":
      "repeating-linear-gradient( 0deg,rgba(168,85,247,0.15) calc(var(--step) * 1),rgba(131,255,247,0.1) calc(var(--step) * 3),rgba(120,148,255,0.15) calc(var(--step) * 5),rgba(168,85,247,0.15) calc(var(--step) * 7) ) 0% var(--bg-y)/200% 700% no-repeat",
    "--diagonal":
      "repeating-linear-gradient( 128deg,#0e152e 0%,hsl(180,10%,60%) 3.8%,hsl(180,10%,60%) 4.5%,hsl(180,10%,60%) 5.2%,#0e152e 10%,#0e152e 12% ) var(--bg-x) var(--bg-y)/300% no-repeat",
    "--shade":
      "radial-gradient( farthest-corner circle at var(--m-x) var(--m-y),rgba(255,255,255,0.04) 12%,rgba(255,255,255,0.06) 20%,rgba(255,255,255,0.1) 120% ) var(--bg-x) var(--bg-y)/300% no-repeat",
  ...
  return (
    <div
      ...
      className="relative isolate [aspect-ratio:17/21] w-full max-w-[320px] transition-transform delay-[var(--delay)] duration-[var(--duration)] ease-[var(--easing)] will-change-transform [contain:layout_style] [perspective:600px] [touch-action:none]"
      ref={refElement}
      onPointerMove={(event) => {
    ...
    >
      <div className="grid h-full origin-center [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] overflow-hidden rounded-[var(--radius)] border border-zinc-800 transition-transform delay-[var(--delay)] duration-[var(--duration)] ease-[var(--easing)] will-change-transform hover:filter-none hover:[--duration:200ms] hover:[--easing:linear] hover:[--opacity:0.3]">
